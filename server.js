...
app.get("/partite", async (req, res) => {
  try {
    ...
    for (const league of LEAGUES) {
      ...
      const filtrate = data.events.map(ev => {
        const dataMatch = new Date(
          ev.dateEvent + "T" + (ev.strTime || "20:00")
        );
        return {
          lega: league.name,
          casa: ev.strHomeTeam,
          ...
          data: ev.dateEvent,
          ora: ev.strTime || "TBD",
          timestamp: dataMatch.getTime()
        };
      });

      tutteLePartite.push(...filtrate);
    }

    // Prima: partite del giorno richiesto
    let risultato = tutteLePartite.filter(
      p => p.timestamp >= start.getTime() && p.timestamp <= end.getTime()
    );

    // Se oggi/domani NON c'è nulla → prendi le prossime future
    if (risultato.length === 0) {
      risultato = tutteLePartite
    ...
    res.status(500).json({ error: "Errore backend calcio" });
  }
});
